...
class FDA:
    ...
    def predict_lda_class(self,X,MU_list,SIGMA,pi_list, y):
        ...
        for p in range(len(classes)):
            score = get_lda_score(X.reshape(-1,1),MU_list[p].reshape(-1,1),SIGMA,pi_list[0]) 
            scores_list.append(score)
        self.FDA_List=scores_list
        return classes[np.argmax(scores_list)]

    def PlayFDA(self,x,y):
        X1 = x.iloc[120, :].values.reshape(4, -1)
        pi_k = [np.count_nonzero(y==1)/len(y) for x in range(3)]
        sigma = x.cov().values
        mean_vectors = []
